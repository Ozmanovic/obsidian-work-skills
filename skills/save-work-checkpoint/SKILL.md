...
description: Save an in-progress software/integration project checkpoint to the user's Obsidian vault from current thread, repo state, or AI agent session evidence. Use when the user says "Save checkpoint", "save work checkpoint", "checkpoint to Obsidian", or asks to preserve where current project work was left off.
---
...
2. Gather checkpoint facts:
   ...
   - Repo state if useful: `git status --short`, `git branch --show-current`, `git diff --stat`, `git diff --name-only`, recent commits.
...
## Current Repo State
<Branch, tracked/untracked files, build/runtime status, exact stopping point, and known incomplete behavior.>

## Open Questions
- <unresolved business/config/technical question; omit this section only if none exist>

## What Still Needs Doing
1. <next code/test/config/customer-question action>
2. <next code/test/config/customer-question action>
