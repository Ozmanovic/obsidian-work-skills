...
Maintain project checkpoint memory in Obsidian from recent AI agent coding work. This skill is for scheduled or manual runs that inspect AI agent threads, git history, and working tree changes, then write fresh project checkpoint notes.
...
- Do not edit source code, commit, push, run deployments, or change production systems.
...
4. Git metadata for repos mentioned in sessions:
   - branch
   - status
   - recent commits
   - changed filenames
   - diffs or source contents only when useful
...
5. Files changed and function/integration names.
...
- <branch/repo/worktree state, changed files, build/test/runtime status if known>
