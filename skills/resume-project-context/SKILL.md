...
description: Resume a software/integration project from Obsidian project summaries, checkpoint notes, repo state, or AI agent session evidence. Use when the user says "Resume project context", "resume this project", "what happened in project X", "where did we leave off", or asks for a concise reminder of achieved work and next steps.
---
...
1. Identify project:
   ...
   - Use current repo name from `git rev-parse --show-toplevel` when available.
...
2. Gather context:
   ...
   - If the newest checkpoint is later than the completed summary, treat the project as active again; use the checkpoint/current repo facts as source of truth and use the completed summary only as background.
   - If the completed summary is later than the newest checkpoint and no newer repo/thread evidence exists, treat the project as completed and resume from the completed summary.
   - If current repo facts or AI agent evidence are newer than both Obsidian notes, say the notes are stale, use the newer facts, and suggest saving a fresh checkpoint if work is continuing.
   - Latest reliable state wins: newest checkpoint/current repo facts override older project summaries.
   - If notes do not include the latest work, inspect relevant AI agent sources from the shared rules.
   - Inspect current repo lightly if useful: branch, status, recent commits, touched files.
...
Where left off:
- <branch/repo state/key files/build/runtime status>
...
- If the checkpoint lacks enough detail for a causal recap, say what is known and fall back to repo/git facts.
...
- If notes are missing, say so and fall back to repo/git facts if available.
